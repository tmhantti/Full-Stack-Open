  ...
  const addPerson = (event) => {
    ...
    if (allNames.includes(newName)) {
      ...
      if (window.confirm(`${newName} is already added to phonebook, replace the old number with new one?`)) {
        ...
        // päivitä serveri ja persons-rakenne
        services
    ...
    // muussa tapauksessa luo uusi objekti
      ...
      // lisää objekti serverille + rakenteeseen
      services
      .create(nameObject)
      .then(returnedPerson => {
      ...
      // virheenkäsittely (tulostetaan mongoosen antama virheilmoitus)
      .catch(error => {
        console.log( (error.message))
        setErrorMessage("Person validation failed: check name and number")
        setTimeout(() => {
          setErrorMessage(null)
        }, 5000)
      })
  ...
  const deletePerson = id => {
    ...
    if (window.confirm(`Delete ${removedPerson.name}?`)) {
      ...
      services
        ...
        // tulosta ilmoitus
        setSuccessMessage(`Person '${removedPerson.name}' was deleted from the phone book`),
        setTimeout(() => {
          setSuccessMessage(null)
        }, 5000))
        // virheilmoitus
        .catch(error => {
          setErrorMessage(`Person '${removedPerson.name}' was already removed from server`)
